...
def get_architecture(dataset):
    if dataset == 'Fdataset':
        best_architecture = 'inter_sage||intra_general||inter'
    elif dataset == 'Cdataset':
        best_architecture = 'inter_general||intra_general||inter'
    elif dataset == 'LRSSL':
        best_architecture = 'inter_sage||intra_graph||inter'
...
if __name__ == "__main__":
    ...
    searched_architecture = get_architecture(config['dataset'])
    print('train from scratch searched architecture ({}) for {}...'.format(searched_architecture, config['dataset']))

    searched_architecture = searched_architecture.split('||')
    auroc, auroc_std, aupr, aupr_std = train_scratch(searched_architecture)

    print('train from scratch ending')
    print('*' * 50)
    print('The testing results for {} as follows:'.format(config['dataset']))
    print('\t\t\t auroc:{:>.5f}, auroc_std:{:>.3f}'.format(auroc, auroc_std))
    print('\t\t\t aupr:{:>.5f}, aupr_std:{:>.3f}'.format(aupr, aupr_std))
    print('\t\t\t achitecture:{}'.format(searched_architecture))
    print('*' * 50)
